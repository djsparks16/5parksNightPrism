...
public:
    ...
    void reset()
    {
        ...
        age = 0; basePan = 0.5f; releaseSamples = 0; postToneL = postToneR = 0.0f;
    }
    ...
    {
        active = true; held = true; midiNote = note; velocity = vel; age = newAge; releaseSamples = 0; postToneL = postToneR = 0.0f;
    ...
    StereoSample render(const RuntimeParams& p)
    {
        ...
        const float sourceLevel = p.oscALevel + p.oscBLevel + p.subLevel + p.noiseLevel;
        ...
        const float gentle = 1.0f - juce::jlimit(0.0f, 1.0f, p.toneAmount * 0.65f + p.airAmount * 0.35f);
        const float toneTilt = juce::jmap(p.toneAmount, 0.0f, 1.0f, 0.22f, 2.15f);
        ...
        const float distDrive = (p.distDrive + modDist * 0.22f + macroWarhorn * 0.14f + p.fmAmount * 0.12f + p.growlAmount * 0.18f + p.toneAmount * 0.08f + macroScreech * 0.12f + macroWhoop * 0.08f) * sourcePresence;
        const float foldAmt = juce::jlimit(0.0f, 1.0f, (p.motionAmount * 0.26f + p.growlAmount * 0.18f + macroWarhorn * 0.12f) * sourcePresence);
        ...
        l = mid + side;
        r = mid - side;
        const float air = (p.airAmount + 0.22f * (p.toneAmount - 0.5f) + 0.18f * macroLaser + 0.14f * macroVapor) * 0.08f * sourcePresence;
        l = std::tanh(l + (l - mid) * air);
        r = std::tanh(r + (r - mid) * air);

        const float smoothCoeff = juce::jlimit(0.02f, 0.55f, 0.06f + p.toneAmount * 0.16f + p.airAmount * 0.12f + macroLaser * 0.08f);
        postToneL += smoothCoeff * (l - postToneL);
        postToneR += smoothCoeff * (r - postToneR);
        l = juce::jmap(0.60f * gentle, l, postToneL);
        r = juce::jmap(0.60f * gentle, r, postToneR);
